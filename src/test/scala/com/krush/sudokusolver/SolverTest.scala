...
class SolverTest extends FunSuite {    
    ...
    test("eliminate infinite loop") {
		new Solver {
		  val gridString = "412300805030000000000700000020000060000080400000010000000603070500200000104000000"
		  val testList = scala.collection.mutable.Map(
		              Pos('A', 1) -> List(4), Pos('A', 2) -> List(1), Pos('A', 3) -> List(2), Pos('A', 4) -> colRef, Pos('A', 5) -> colRef, Pos('A', 6) -> colRef, Pos('A', 7) -> List(8), Pos('A', 8) -> colRef, Pos('A', 9) -> List(5),
